...
export function Tasks() {
  ...
  const { data, hasNextPage, fetchNextPage, isFetchingNextPage, refetch } =
    useInfiniteQuery<PaginatedData<Task>>({
      ...
      queryFn: ({ pageParam }) =>
        currentParentId
          ? getPaginatedSubTask(currentParentId, pageParam as number)
          : getPaginatedTask(pageParam as number),
          ...
          <div className="w-full px-5 py-5 h-[50vh] ">
            <Virtuoso
                  ...
                  return (
                    <div className="text-center py-12">
                      ...
                      <Button
                        ...
                        className="mt-4 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded-lg transition duration-300 ease-in-out hover:scale-110"
                      >
                        Ajouter une tâche
                      </Button>
                    </div>
              ...
              itemContent={(_, item) => (
                <div className="py-2">
                  <TaskItem
                    isExpand={!currentTaskId}
                    task={item}
                    onPress={() => setCurrentTaskId(item.id)}
                    onDelete={() => {
