...
export function TaskDetail({
  ...
  const [showDescriptionAddBtn, setShowDescriptionAddBtn] =
    useState<boolean>(false);
  const [showAddDescritpionInput, setShowAddDescriptionInput] =
    useState<boolean>(false);
  const [editTitle, setEditTitle] = useState<boolean>(false);
  const deleteMutation = useMutation({
    mutationKey: [TASK, "delete", taskId],
    mutationFn: ({ taskId: subTaskId }: { taskId: string }) =>
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage,
  } = useInfiniteQuery<PaginatedData<Task>>({
    queryKey: [SUB_TASKS, taskId],
    queryFn: ({ pageParam }) =>
      getPaginatedSubTask(taskId, pageParam as number),
  ...
  if (taskResult.isPending) {
    return (
      <div className="w-full h-full  items-center justify-center bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl flex flex-col overflow-hidden text-slate-400">
        <Spinner />
      </div>
    );
  }
  ...
  return (
    <>
      {" "}
      <div className=" w-full h-full bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl flex flex-col overflow-hidden">
          ...
          >
            <div className={`flex items-start justify-between `}>
                ...
                <h2
                  onBlur={(e) => {
                    updateMutation.mutate({
                      taskId,
                      dto: { title: e.currentTarget.textContent },
                    });
                    setEditTitle(false);
                  }}
                  onKeyDown={(e) => {
                  ...
                  className={`text-2xl font-bold break-all ${editTitle && "border-slate-700/40 border p-1 rounded-lg"} `}
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  onClick={onClose}
                  className=" duration-300 ease-in-out hover:scale-110 p-2 text-gray-50 hover:bg-gray-500/20 rounded-lg transition"
                  title="Close"
                >
                  <X className="h-5 w-5" />
                </Button>
            ...
            <Virtuoso
                      ...
                      {(hasDescription || showAddDescritpionInput) && (
                        <div className="mb-6">
                          ...
                          <p
                            ...
                            className={`text-slate-300 leading-relaxed ${showAddDescritpionInput && "border-slate-700/40 border p-1 rounded-lg"}`}
                            contentEditable={showAddDescritpionInput}
                            suppressContentEditableWarning
                          >
                            {task.description}
                          </p>
                  ...
                  return (
                    <div className="text-center py-12">
                      ...
                      <p className="text-slate-400">
                        Cette tâche n'a pas de sous-tâches
                      </p>
                      <Button
                        onClick={() => setShowAddSubTaskModal(true)}
                        className="duration-300 ease-in-out hover:scale-110mt-4 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded-lg transition"
                      >
              ...
              itemContent={(_, item) => (
                <div className="py-2">
                  <TaskItem
                    task={item}
                    onPress={() => onPressSubTask && onPressSubTask(item.id)}
                    onDelete={() => {
          ...
          <div className="p-6 border-t border-slate-700">
            <div className="flex space-x-3">
              ...
              >
                <SelectTrigger
                  withIcon={false}
                  className={` duration-300 ease-in-out hover:scale-101 px-6 py-3 min-h-12 text-white   rounded-lg font-semibold transition ${getStatusColor(task.status)}`}
                >
